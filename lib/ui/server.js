...
function sanitizeErrorMessage(settings, sessionToken, serviceOptions, error) {
  let message = String(error && error.message || error || 'Unknown error');
  const values = [settings.homeDir, sessionToken]
    .concat(Array.isArray(serviceOptions.auditSecrets) ? serviceOptions.auditSecrets : [])
    .filter(Boolean).map(String).sort(function longestFirst(left, right) { return right.length - left.length; });
  values.forEach(function redact(value) {
...
function createUiServer(settings, options) {
  ...
  const serviceOptions = Object.assign({}, opts.serviceOptions || {});
  serviceOptions.auditSecrets = (Array.isArray(serviceOptions.auditSecrets) ? serviceOptions.auditSecrets : []).concat([sessionToken]);
  const service = opts.service || createUiService(settings, serviceOptions);
  ...
  const server = http.createServer(function handle(request, response) {
    Promise.resolve().then(async function route() {
      ...
      if (request.method === 'POST' && pathname === '/api/skills/link/preview') {
        requireSession(request, sessionToken);
        sendJson(response, 200, service.previewSkillLink(await readJsonBody(request)));
        return;
      }
      if (request.method === 'POST' && pathname === '/api/skills/link/apply') {
        requireSession(request, sessionToken);
        sendJson(response, 200, service.applySkillLink(await readJsonBody(request)));
        return;
      }
      if (request.method === 'POST' && pathname === '/api/skills/unlink/preview') {
        requireSession(request, sessionToken);
        sendJson(response, 200, service.previewSkillUnlink(await readJsonBody(request)));
        return;
      }
      if (request.method === 'POST' && pathname === '/api/skills/unlink/apply') {
        requireSession(request, sessionToken);
        sendJson(response, 200, service.applySkillUnlink(await readJsonBody(request)));
        return;
      }
      if (request.method === 'POST' && pathname === '/api/skills/removal/preview') {
        requireSession(request, sessionToken);
        sendJson(response, 200, service.previewSkillRemoval(await readJsonBody(request)));
        return;
      }
      if (request.method === 'POST' && pathname === '/api/skills/removal/apply') {
        requireSession(request, sessionToken);
        sendJson(response, 200, service.applySkillRemoval(await readJsonBody(request)));
        return;
      }
      if (request.method === 'POST' && pathname === '/api/skills/removal/rollback/preview') {
        requireSession(request, sessionToken);
        sendJson(response, 200, service.previewSkillRemovalRollback(await readJsonBody(request)));
        return;
      }
      if (request.method === 'POST' && pathname === '/api/skills/removal/rollback/apply') {
        requireSession(request, sessionToken);
        sendJson(response, 200, service.applySkillRemovalRollback(await readJsonBody(request)));
        return;
      }
      if (request.method === 'POST' && pathname === '/api/skills/removal/purge/preview') {
        requireSession(request, sessionToken);
        sendJson(response, 200, service.previewSkillRemovalPurge(await readJsonBody(request)));
        return;
      }
      if (request.method === 'POST' && pathname === '/api/skills/removal/purge/apply') {
        requireSession(request, sessionToken);
        sendJson(response, 200, service.applySkillRemovalPurge(await readJsonBody(request)));
        return;
      }
      if (request.method === 'POST' && pathname === '/api/skills/removal/bulk-purge/preview') {
        requireSession(request, sessionToken);
        await readJsonBody(request);
        sendJson(response, 200, service.previewSkillRemovalBulkPurge());
        return;
      }
      if (request.method === 'POST' && pathname === '/api/skills/removal/bulk-purge/apply') {
        requireSession(request, sessionToken);
        sendJson(response, 200, service.applySkillRemovalBulkPurge(await readJsonBody(request)));
        return;
      }
      ...
      if (request.method === 'GET' && pathname === '/api/updates/source/popular/progress') {
        sendJson(response, 200, service.popularApplyProgress(selected.searchParams.get('plan_id')));
        return;
      }
      if (request.method === 'POST' && pathname === '/api/updates/source/popular/discover') {
        requireSession(request, sessionToken);
        sendJson(response, 200, await service.discoverPopularSkillSources(await readJsonBody(request)));
        return;
      }
      if (request.method === 'POST' && pathname === '/api/updates/source/popular/preview') {
        requireSession(request, sessionToken);
        sendJson(response, 200, await service.previewPopularSkillSources(await readJsonBody(request)));
        return;
      }
      if (request.method === 'POST' && pathname === '/api/updates/source/popular/apply') {
        requireSession(request, sessionToken);
        sendJson(response, 200, await service.applyPopularSkillSources(await readJsonBody(request)));
        return;
      }
      ...
      if (request.method === 'POST' && pathname === '/api/snapshots/open-directory') {
        requireSession(request, sessionToken);
        await readJsonBody(request);
        sendJson(response, 200, service.openSnapshotDirectory());
        return;
      }
      ...
      sendJson(response, statusCode, {
        error: {
          ...
          message: sanitizeErrorMessage(settings, sessionToken, serviceOptions, error),
        },
      });
    });
  });
