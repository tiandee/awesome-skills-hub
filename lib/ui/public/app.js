...
(function boot() {
  ...
  function updatePresentation(update) {
    return (update && update.display) || { key: 'error', label: '状态异常', tone: 'danger', detail: '缺少状态展示信息' };
  }
  ...
  function renderLibrary() {
    ...
    container.innerHTML = visible.map(function row(skill) {
      ...
      const linking = skill.linking || { available: false, label: '链接到用户库' };
      const unlinking = skill.unlinking || { available: false, label: '解除链接' };
      const menuItems = removal.available
        ? '<button type="button" role="menuitem" data-action="preview-package" data-skill="' + escapeHtml(skill.name) + '" data-library-id="' + escapeHtml(skill.library_id) + '">打包 .skill</button>' +
          (unlinking.available ? '<button type="button" role="menuitem" data-action="preview-skill-unlink" data-skill="' + escapeHtml(skill.name) + '" data-library-id="' + escapeHtml(skill.library_id) + '">' + escapeHtml(unlinking.label) + '</button>' : '') +
        ...
        : (linking.available ? '<button type="button" role="menuitem" data-action="preview-skill-link" data-skill="' + escapeHtml(skill.name) + '" data-library-id="' + escapeHtml(skill.library_id) + '">' + escapeHtml(linking.label) + '</button>' : '') +
  ...
  function renderSkillDetail() {
    ...
    container.innerHTML =
      ...
      '<div class="detail-toolbar">' + (skill.can_write
        ...
        (skill.can_link ? '<button class="primary-button" type="button" data-action="preview-skill-link" data-skill="' + escapeHtml(skill.name) + '" data-library-id="' + escapeHtml(skill.library_id) + '">链接到用户库</button>' : '') +
        (skill.can_unlink ? '<button class="secondary-button" type="button" data-action="preview-skill-unlink" data-skill="' + escapeHtml(skill.name) + '" data-library-id="' + escapeHtml(skill.library_id) + '">解除链接</button>' : '') +
        (skill.can_remove ? '<button class="destructive-outline-button" type="button" data-action="preview-skill-removal" data-skill="' + escapeHtml(skill.name) + '" data-library-id="' + escapeHtml(skill.library_id) + '">' + escapeHtml((skill.removal || {}).label || '移入回收站') + '</button>' : '') + '</div>' +
  ...
  function renderMaintenance() {
    ...
    document.getElementById('update-card').innerHTML =
      ...
      '<ul class="action-list"><li><span>等待检查</span><span>' + updates.summary.eligible + '</span></li><li><span>发现更新</span><span>' + updates.summary.update_available + '</span></li><li><span>已是最新</span><span>' + updates.summary.up_to_date + '</span></li><li><span>等待接管</span><span>' + updates.summary.unmanaged + '</span></li><li><span>等待重建</span><span>' + updates.summary.baseline_missing + '</span></li></ul>' +
  ...
  const confirmations = {
    ...
    'skill-link': { eyebrow: 'USER LIBRARY LINK / PREVIEW FIRST', title: '确认链接到用户库', description: '将在受管用户库中创建指向只读源目录的软链接。不会复制或修改源目录；确认时会重新校验源内容和目标路径。', apply: '创建用户链接', route: '/api/skills/link/apply', id: 'plan_id', busy: '正在创建链接…', success: 'Skill 已链接到用户库。' },
    'skill-unlink': { eyebrow: 'USER LIBRARY LINK / REVERSE', title: '确认解除用户链接', description: '只删除用户库中的软链接，不修改源目录，也不会移入回收站。之后仍可从只读来源重新链接。', apply: '解除链接', route: '/api/skills/unlink/apply', id: 'plan_id', busy: '正在解除链接…', success: '用户库链接已解除。' },
    'skill-removal': { eyebrow: 'SKILL RECYCLE BIN / ONE SKILL', title: '确认移入回收站', description: '目录或链接将进入 ASH 回收站事务，不会立即永久删除；安装器锁条目会在同一事务中更新。', apply: '移入回收站', route: '/api/skills/removal/apply', id: 'plan_id', busy: '正在移入回收站…', success: 'Skill 已移入回收站。', confirmName: true },
    ...
    'skill-removal-purge': { eyebrow: 'PERMANENT DELETE / IRREVERSIBLE', title: '确认永久删除', description: '将永久删除回收站中的 Skill 内容与事务记录。此操作无法撤销，也不能再从 ASH 恢复。', apply: '永久删除', route: '/api/skills/removal/purge/apply', id: 'plan_id', busy: '正在永久删除…', success: 'Skill 与回收站事务已永久删除。', confirmName: true },
    'skill-removal-bulk-purge': { eyebrow: 'DELETE ALL / IRREVERSIBLE', title: '确认永久删除全部', description: '将永久删除当前回收站中的全部 Skill 内容和事务记录。任何一项在确认前发生变化，整批预览都会失效。', apply: '永久删除全部', route: '/api/skills/removal/bulk-purge/apply', id: 'plan_id', busy: '正在永久删除全部…', success: '回收站已全部永久删除。', confirmText: true },
  ...
  async function previewSkillLink(name, libraryId) {
    state.skillMenuKey = null;
    renderLibrary();
    setActionBusy('preview-skill-link', true, '正在核对…');
    try {
      const preview = await post('/api/skills/link/preview', { name, library_id: libraryId });
      openConfirmation('skill-link', preview);
    } catch (error) { showToast(error.message, true); }
    finally { setActionBusy('preview-skill-link', false); }
  }

  async function previewSkillUnlink(name, libraryId) {
    state.skillMenuKey = null;
    renderLibrary();
    setActionBusy('preview-skill-unlink', true, '正在核对…');
    try {
      const preview = await post('/api/skills/unlink/preview', { name, library_id: libraryId });
      openConfirmation('skill-unlink', preview);
    } catch (error) { showToast(error.message, true); }
    finally { setActionBusy('preview-skill-unlink', false); }
  }
  ...
  async function applyConfirmation() {
    ...
    try {
      ...
      await loadOverview({ silent: true });
      if ((modal.kind === 'skill-create' || modal.kind === 'skill-edit' || modal.kind === 'skill-source' || modal.kind === 'skill-update' || modal.kind === 'skill-update-rollback' || modal.kind === 'skill-removal-rollback' || modal.kind === 'skill-link') && result.skill) {
  ...
  document.addEventListener('click', function delegated(event) {
    ...
    else if (name === 'preview-skill-link') previewSkillLink(action.dataset.skill, action.dataset.libraryId);
    else if (name === 'preview-skill-unlink') previewSkillUnlink(action.dataset.skill, action.dataset.libraryId);
