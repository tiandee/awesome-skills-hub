...
function helpText() {
  return [
    ...
    '  repair [--apply] [--json]           Preview or apply deterministic safe repairs',
    ...
    '',
    'Repair is dry-run by default. It never overwrites non-symlinks, valid links',
    'owned by another source, Codex Store data, system Skills, or plugin caches.',
  ].join('\n');
}
...
function selectPackageSkills(settings, names, allSkills) {
  ...
  if (missing.length > 0) throw new Error('unknown ASH skills: ' + missing.join(', '));
  const ambiguous = names.filter(function duplicate(name) { return byName.get(name).length !== 1; });
  if (ambiguous.length > 0) throw new Error('ambiguous ASH skills: ' + ambiguous.join(', '));
...
function main(argv, options) {
  ...
  try {
    ...
    if (command === 'repair') {
      ...
      const plan = buildRepairPlan(settings);
      if (json && !apply) {
