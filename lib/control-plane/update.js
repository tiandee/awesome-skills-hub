...
const UPDATE_ERROR_MESSAGE_LIMIT = 400;
...
function githubLockIdentity(entry, options) {
  if (!entry || entry.sourceType !== 'github' || !entry.sourceUrl || !entry.skillPath) return null;
  return {
    source: String(entry.source || sourceName(entry.sourceUrl)),
    sourceUrl: validateSourceUrl(entry.sourceUrl, options),
    skillPath: canonicalSkillPath(entry.skillPath),
    ref: validateRef(entry.ref),
  };
}

function sameGithubLockIdentity(left, right) {
...
async function buildSkillSourcePreview(settings, input, options) {
  ...
  if (skill.status !== 'unmanaged' && skill.status !== 'baseline-missing' && skill.status !== 'checkable') {
  ...
  const retargeting = skill.status === 'checkable';
  ...
  if (retargeting && (!previousEntry || !eligibleLockEntry(previousEntry))) {
    throw new Error('existing Skill source does not support retargeting');
  }
  if (retargeting && !(request.skills_url || request.skillsUrl || request.source_url || request.sourceUrl)) {
    throw new Error('retargeting a Skill source requires a new skills.sh URL or GitHub source');
  }
  let catalog = null;
  let sourceUrl;
  let skillPath;
  ...
  const previousIdentity = githubLockIdentity(previousEntry, opts);
  if (retargeting && sameGithubLockIdentity(previousIdentity, { sourceUrl, skillPath, ref })) {
    throw new Error('new update source is the same as the current upstream');
  }
  ...
  try {
    ...
    return Object.assign({
      operation: rebuilding ? 'rebuild-baseline' : retargeting ? 'retarget-source' : 'link-source',
      name: skill.name,
      path: skill.path,
      source,
      ...
      ref,
      previous_source: retargeting && previousIdentity ? previousIdentity.source : null,
      previous_source_url: retargeting && previousIdentity ? previousIdentity.sourceUrl : null,
      previous_skill_path: retargeting && previousIdentity ? previousIdentity.skillPath : null,
      previous_ref: retargeting && previousIdentity ? previousIdentity.ref : null,
...
function updateErrorSanitizer(settings, options) {
  const opts = options || {};
  const homeDir = String(settings.homeDir || '');
  const redactionValues = [homeDir].concat(Array.isArray(opts.redactionValues) ? opts.redactionValues : [])
    .filter(Boolean).map(String).sort(function longestFirst(left, right) { return right.length - left.length; });
  return function sanitize(error) {
    let message = String(error && error.message || error || 'Unknown error');
    redactionValues.forEach(function redact(value) {
      const replacement = value === homeDir ? '<HOME>' : '<REDACTED>';
      message = message.split(value).join(replacement);
      const alternate = value.replace(/\\/g, '/');
      if (alternate !== value) message = message.split(alternate).join(replacement);
    });
    message = message
      ...
      .replace(/\s+/g, ' ')
      .trim();
    if (message.length > UPDATE_ERROR_MESSAGE_LIMIT) message = message.slice(0, UPDATE_ERROR_MESSAGE_LIMIT - 1) + '…';
    return message;
  };
}

function annotateUpdateError(error, phase, transactionId, sanitize, rollbackFailed) {
  const selected = error && typeof error === 'object' ? error : new Error(String(error));
  selected.message = sanitize(selected);
  selected.ashPhase = phase;
  if (transactionId) selected.ashTransactionId = transactionId;
  if (rollbackFailed) selected.ashRollbackFailed = true;
  return selected;
}
...
function notifyUpdateRollback(options, details) {
  if (!options || typeof options.onRollback !== 'function') return;
  try {
    options.onRollback(details);
  } catch (error) {
    // Observability callbacks must never affect transaction restoration.
  }
}

function applyUpdateTransaction(settings, plan, candidate, options) {
  const opts = options || {};
  const sanitizeError = updateErrorSanitizer(settings, opts);
  ...
  let lock;
  let previousLockEntry;
  let current;
  try {
    lock = lockPayload(settings);
    previousLockEntry = lock.payload.skills[plan.name] || null;
    ...
    current = portableSkillState(plan.path, plan.name);
    ...
    throw annotateUpdateError(error, 'preview_validation', null, sanitizeError, false);
  }
  try {
    fs.mkdirSync(updateTransactionRoot(settings), { recursive: true, mode: 0o700 });
    fs.mkdirSync(transactionDir, { mode: 0o700 });
  } catch (error) {
    throw annotateUpdateError(error, 'preparation', null, sanitizeError, false);
  }
  const payload = {
  ...
  try {
    ...
    throw annotateUpdateError(error, 'preparation', id, sanitizeError, false);
  }
  ...
  try {
    ...
    // Rebase the transaction on the latest lock immediately before changing local state.
    // Only this Skill's confirmed entry is protected; unrelated concurrent entries are retained.
    const latestLock = lockPayload(settings);
    previousLockEntry = latestLock.payload.skills[plan.name] || null;
    if (entryDigest(previousLockEntry) !== plan.lock_entry_hash) {
    ...
    payload.previous_lock_entry_exists = Boolean(previousLockEntry);
    writeUpdateTransaction(transactionFile, payload);
    ...
    const nextLock = latestLock.payload;
    ...
    const nextLockContent = Buffer.from(JSON.stringify(nextLock, null, 2) + '\n', 'utf8');
    if (lockPayload(settings).hash !== latestLock.hash) throw new Error('Agents installer lock changed immediately before write');
    atomicWrite(settings.agentsLock, nextLockContent);
    lockWritten = true;
    writtenLockHash = sha256(nextLockContent);
    payload.lock_written = true;
    payload.written_lock_sha256 = writtenLockHash;
    const writtenLock = verifyWrittenLockEntry(settings, plan);
    if (writtenLock.hash !== writtenLockHash) throw new Error('Agents installer lock changed after transaction write');
    ...
    payload.rollback = {
      initiator: String(opts.rollbackInitiator || 'update_transaction'),
      reason: sanitizeError(opts.rollbackReason || error),
      outcome: 'in_progress',
      started_at: rollbackStartedAt,
      completed_at: null,
      failed_at: null,
    };
    try { writeUpdateTransaction(transactionFile, payload); } catch (metadataError) {
      // Failure metadata is best effort until the original state has been restored.
    }
    notifyUpdateRollback(opts, {
      transaction_id: id,
      initiator: payload.rollback.initiator,
      reason: error,
      outcome: 'started',
      timestamp: rollbackStartedAt,
    });
    try {
      if (lockWritten && lockPayload(settings).hash !== writtenLockHash) {
        throw new Error('Agents installer lock changed after transaction write; refusing automatic lock rollback');
      }
      ...
      if (lockWritten) atomicWrite(settings.agentsLock, fs.readFileSync(lockBackup));
    ...
    if (rollbackError) {
      ...
      payload.rollback_error = sanitizeError(rollbackError);
      payload.rollback.outcome = 'failed';
      payload.rollback.failed_at = new Date().toISOString();
    } else {
      payload.rollback.outcome = 'completed';
      payload.rollback.completed_at = new Date().toISOString();
    }
    try {
      writeUpdateTransaction(transactionFile, payload);
    } catch (metadataError) {
      throw annotateUpdateError(new Error(payload.error + '; transaction metadata write failed: ' + sanitizeError(metadataError)),
        'local_apply', id, sanitizeError, Boolean(rollbackError));
    }
    notifyUpdateRollback(opts, {
      transaction_id: id,
      initiator: payload.rollback.initiator,
      reason: error,
      outcome: payload.rollback.outcome,
      timestamp: payload.rollback.completed_at || payload.rollback.failed_at,
      rollback_error: rollbackError,
    });
    if (error && typeof error === 'object') {
      error.message = payload.error;
      error.ashPhase = 'local_apply';
      error.ashTransactionId = id;
    }
    if (rollbackError) {
      throw annotateUpdateError(new Error('update failed: ' + payload.error + '; rollback failed: ' + payload.rollback_error),
        'local_apply', id, sanitizeError, true);
    }
    throw annotateUpdateError(error, 'local_apply', id, sanitizeError, false);
  }
}
...
async function applySkillUpdate(settings, plan, options) {
  ...
  let materialized;
  try {
    freshPlan = await buildSkillUpdatePreview(settings, {
      name: plan.name,
      latest_hash: plan.latest_hash,
      latest_revision: plan.latest_revision,
    }, { sourceClient });
    if (updatePlanDigest(freshPlan) !== updatePlanDigest(plan)) throw new Error('update preview is stale');
    materialized = await sourceClient.materialize({
      name: plan.name,
    ...
    throw annotateUpdateError(error, /stale|changed after/.test(String(error.message || '')) ? 'preview_validation' : 'source_refresh', null, sanitizeError, false);
  }
  try {
    ...
    return applyUpdateTransaction(settings, plan, candidate, opts);
  } catch (error) {
    if (error && error.ashPhase) throw error;
    throw annotateUpdateError(error, /changed after/.test(String(error.message || '')) ? 'preview_validation' : 'preparation', null, sanitizeError, false);
  } finally {
    materialized.cleanup();
  }
}
...
async function applySkillSource(settings, plan, options) {
  ...
  let materialized;
  try {
    freshPlan = await buildSkillSourcePreview(settings, {
      name: plan.name,
      source: plan.source,
      skills_url: plan.skills_url,
...
function previewSkillUpdateRollback(settings, selector) {
  ...
  } else if (payload.operation === 'retarget-source') {
    description = 'RESTORE SKILL ' + payload.name + ' AND ITS PREVIOUS UPDATE SOURCE';
  }
  return {
...
function applySkillUpdateRollback(settings, selector, options) {
  const opts = options || {};
  const sanitizeError = updateErrorSanitizer(settings, opts);
  ...
  const updatedLock = fs.readFileSync(settings.agentsLock);
  let currentMoved = false;
  let originalRestored = false;
  payload.rollback = Object.assign(rollbackMetadata(), payload.rollback || {}, {
    initiator: String(opts.initiator || 'manual'),
    reason: sanitizeError(opts.reason || 'manual_request'),
    outcome: 'in_progress',
    started_at: new Date().toISOString(),
    completed_at: null,
    failed_at: null,
  });
  delete payload.rollback.error;
  delete payload.rollback.recovery_error;
  try {
    writeUpdateTransaction(transaction.transactionFile, payload);
  } catch (error) {
    throw annotateUpdateError(error, 'manual_rollback', payload.id, sanitizeError, false);
  }
  try {
    if (contentReplaced) {
    ...
    payload.rollback.outcome = 'completed';
    payload.rollback.completed_at = payload.rolled_back_at;
    writeUpdateTransaction(transaction.transactionFile, payload);
    ...
    try {
      ...
      atomicWrite(settings.agentsLock, updatedLock);
    } catch (caught) {
      recoveryError = caught;
    }
    payload.status = 'completed';
    delete payload.rolled_back_at;
    delete payload.updated_backup_path;
    payload.rollback.outcome = 'failed';
    payload.rollback.failed_at = new Date().toISOString();
    payload.rollback.error = sanitizeError(error);
    if (recoveryError) payload.rollback.recovery_error = sanitizeError(recoveryError);
    try { writeUpdateTransaction(transaction.transactionFile, payload); } catch (metadataError) {
      // Preserve the original rollback failure when metadata persistence also fails.
    }
    if (recoveryError) throw annotateUpdateError(new Error('rollback failed: ' + payload.rollback.error + '; recovery failed: ' + payload.rollback.recovery_error),
      'manual_rollback', payload.id, sanitizeError, true);
    throw annotateUpdateError(error, 'manual_rollback', payload.id, sanitizeError, true);
  }
}
