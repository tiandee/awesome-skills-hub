...
- 将只读来源中的 Skill 预览后链接到用户库；只创建用户库软链接，不复制或修改源目录，
  并可通过“解除链接”执行直接反向操作，不产生恢复记录；
...
- 从列表操作菜单将受管 Skill“移入回收站”：实体目录或链接进入 ASH 回收站事务，
  链接源目录不受影响；安装器锁条目在同一事务中更新；维护页列出全部可恢复
  记录，可逐项恢复或在二次预览和名称确认后永久删除；也可预览后一键永久删除全部，
  但必须输入包含当前数量的完整确认文字；
- 创建、校验并增量恢复页面受管的用户库快照；
- 从页面直接打开本地快照目录，不执行恢复或删除；
- 展示来源覆盖率、异常来源、缺少基线与长期未更新统计；
- 将更新状态收敛为8个四字状态，并将错误/警告/提示作为独立健康维度展示；
- 在保护当前可回滚事务的前提下，预览并清理过期或已废弃事务；
- 为未关联来源的 Skill 自动查找 skills.sh 精确同名候选，经人工选择后通过准确 URL
  （或 GitHub 仓库与路径）接管，并为旧安装记录安全重建版本基线；
- 按安装量为明显领先的候选生成一次性批量接管预览；歧义、不可用或含可执行文件的
  候选会跳过，仍需一次确认后才写入；
- 区分 skills.sh 接管渠道与实际 GitHub 代码上游，并提供目录页、仓库和精确
  `SKILL.md` 源码链接；
- 对已锁定来源的 Skill 更换上游仓库：先预览文件差异，确认后改绑，并可用最近一次
  更新事务回滚；
...
不可回滚，因此必须先预览再确认：Repair、Update 与 Removal 历史分别保留最近10条
或30天内记录，并始终保护当前仍可安全回滚或恢复的事务。
...
允许绑定回环地址，也不会开放 CORS。
仓库同步、无来源第三方安装、直接编辑指令正文，以及绕过回收站直接永久删除当前用户库
仍不进入页面。页面的“移除”只作用于受管用户库，并始终先进入回收站事务；只有回收站
中的副本可以经单独预览和文字确认后永久删除。自定义扫描来源只能停止扫描整个目录。
