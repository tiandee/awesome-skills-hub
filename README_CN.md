...
- 从列表操作菜单预览并移除受管 Skill：实体目录进入 ASH 可恢复区，软链接只解除
  用户库入口且不修改源目录；安装器锁条目在同一事务中更新；维护页列出全部可恢复
...
允许绑定回环地址，也不会开放 CORS。
仓库同步、无来源第三方安装、直接编辑指令正文，以及绕过恢复区直接永久删除当前用户库
仍不进入页面。页面的“移除”只作用于受管用户库，并始终先进入可恢复事务；只有恢复区
