...
const { isDirectory, lexists, listSkillFiles } = require('./util');
...
function metadataIssues(settings, library) {
  ...
  byName.forEach(function reportDuplicate(paths, name) {
    if (paths.length > 1) {
      issues.push(issue(
        ...
        name + ' appears in multiple ASH library paths',
        paths,
      ));
    }
  });
  if (missingOpenai.length > 0) {
    issues.push(issue(
      ...
      missingOpenai.length + ' ASH skills do not have optional agents/openai.yaml metadata',
      missingOpenai,
    ));
  }
  return issues;
...
function runtimeIssues(inventory) {
  ...
  sourcesByName.forEach(function report(sources, name) {
    if (sources.has('ash-library') && sources.has('third-party')) {
      issues.push(issue(
        'WARN',
        'MULTIPLE_MANAGERS',
        name + ' is owned by both ASH and the third-party lock',
...
function runDoctor(settings) {
  if (!isDirectory(settings.libraryRoot)) {
    return [issue(
      ...
      'ASH Skill library is not present: ' + settings.libraryRoot,
  ...
  if (inventory.library.length === 0) {
    issues.push(issue('WARN', 'ASH_LIBRARY_EMPTY', 'ASH Skill library contains no directory Skills', [settings.libraryRoot]));
  }
  ...
  issues.push.apply(issues, runtimeIssues(inventory));
