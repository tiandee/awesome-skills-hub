...
function discoverLibrary(settings) {
  ...
  function visit(current, relativeParts) {
    ...
    entries.forEach(function inspect(entry) {
      ...
      visit(path.join(current, entry.name), relativeParts.concat(entry.name));
    });
  }
...
function agentsRoot(settings) {
  const configured = settings.targets.find(function find(target) { return target.name === 'generic-agents'; });
  return configured ? configured.path : path.join(path.dirname(settings.agentsLock), 'skills');
}
...
function buildInventory(settings) {
  ...
  if (isDirectory(root)) {
    fs.readdirSync(root, { withFileTypes: true }).sort(function byName(a, b) {
      ...
      } else if (fs.existsSync(path.join(installPath, 'SKILL.md'))) {
        records.push({
          name: entry.name,
          source: 'untracked-agents',
          status: 'untracked',
