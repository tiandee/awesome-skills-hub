...
function buildRepairPlan(settings) {
  if (!isDirectory(settings.libraryRoot)) {
    throw new Error('ASH Skill library is not present: ' + settings.libraryRoot);
  }
  ...
  pathsByName.forEach(function report(paths, name) {
    if (paths.length > 1) {
      ...
      conflicts.push(issue(
        ...
        'refusing to choose between multiple ASH library entries named ' + name,
        paths,
      ));
    }
  });
    ...
    desired.forEach(function planSkill(skill) {
      ...
      if (current.status === 'missing') {
        actions.push({
          kind: 'symlink_create',
          path: installPath,
          target: canonicalPath(skill.path),
        });
