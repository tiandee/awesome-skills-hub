...
const UPDATE_PRESENTATION = Object.freeze({
  'checkable': { key: 'check', label: '等待检查', tone: 'neutral', detail: '来源完整，尚未检查', action: 'check-updates' },
  'up-to-date': { key: 'current', label: '已是最新', tone: 'info', detail: '已与上游一致', action: null },
  'update-available': { key: 'available', label: '发现更新', tone: 'success', detail: '上游存在新内容', action: 'preview-update' },
  'unmanaged': { key: 'takeover', label: '等待接管', tone: 'warning', detail: '尚未关联更新来源', action: 'link-source' },
  'baseline-missing': { key: 'baseline', label: '等待重建', tone: 'warning', detail: '来源已知但缺少可比较基线', action: 'rebuild-baseline' },
  'repository-linked': { key: 'external', label: '用户链接', tone: 'neutral', detail: '由用户维护的本地链接', action: null },
  'read-only-source': { key: 'external', label: '只读来源', tone: 'neutral', detail: '来自只读扫描目录', action: null },
  'source-unavailable': { key: 'error', label: '状态异常', tone: 'danger', detail: '更新来源不可用', action: 'check-source' },
  'missing': { key: 'error', label: '状态异常', tone: 'danger', detail: '安装记录存在但本地缺失', action: 'inspect-missing' },
});
...
function presentUpdateStatus(update) {
  ...
  return { key: 'error', label: '状态异常', tone: 'danger', detail: '无法识别内部状态：' + (status || 'empty'), action: 'inspect-status' };
}
