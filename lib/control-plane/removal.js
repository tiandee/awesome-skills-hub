...
const { NAME_PATTERN, parseSkill } = require('./discovery');
...
function copyRawTree(source, destination) {
  ...
  if (!stat.isFile()) throw new Error('unsupported Skill entry while moving to the recycle bin: ' + source);
...
function buildSkillLinkPlan(settings, input) {
  const request = input || {};
  const name = String(request.name || '').trim();
  if (!NAME_PATTERN.test(name)) throw new Error('invalid user Skill name: ' + name);
  const sourcePath = path.resolve(String(request.source_path || ''));
  if (path.basename(sourcePath) !== name || !fs.existsSync(path.join(sourcePath, 'SKILL.md'))) {
    throw new Error('read-only source is not a readable matching Skill: ' + name);
  }
  const source = parseSkill(sourcePath, name);
  if (source.parseErrors.length) throw new Error('read-only source has invalid SKILL.md metadata: ' + source.parseErrors.join('; '));
  if (source.declaredName && source.declaredName !== name) throw new Error('read-only source declares a different Skill name: ' + source.declaredName);
  const physicalSourcePath = fs.realpathSync(sourcePath);
  const sourceState = entryState(physicalSourcePath);
  if (!sourceState.exists || sourceState.type !== 'directory') throw new Error('read-only Skill source is not a directory: ' + sourcePath);
  const destination = path.join(settings.libraryRoot, name);
  if (path.dirname(destination) !== path.resolve(settings.libraryRoot)) throw new Error('Skill link target escapes the managed user library');
  const destinationState = entryState(destination);
  if (destinationState.exists) throw new Error('managed user library already contains: ' + name);
  return {
    name,
    source_path: sourcePath,
    physical_source_path: physicalSourcePath,
    source_state: sourceState,
    destination,
    destination_state: destinationState,
    actions: [{
      kind: 'skill_link_create',
      path: destination,
      description: '链接只读 Skill 到用户库 ' + sourcePath + ' -> ' + destination,
    }],
  };
}

function skillLinkPlanDigest(plan) {
  return sha256(Buffer.from(JSON.stringify({
    name: plan.name,
    source_path: plan.source_path,
    physical_source_path: plan.physical_source_path,
...
function applySkillLink(settings, plan) {
  const fresh = buildSkillLinkPlan(settings, { name: plan.name, source_path: plan.source_path });
  if (skillLinkPlanDigest(fresh) !== skillLinkPlanDigest(plan)) throw new Error('Skill link preview is stale');
  fs.mkdirSync(settings.libraryRoot, { recursive: true, mode: 0o700 });
  let created = false;
  try {
    fs.symlinkSync(
      fresh.physical_source_path,
      fresh.destination,
      process.platform === 'win32' ? 'junction' : 'dir',
    );
    created = true;
    if (!fs.lstatSync(fresh.destination).isSymbolicLink() ||
        fs.realpathSync(fresh.destination) !== fresh.physical_source_path ||
        !fs.existsSync(path.join(fresh.destination, 'SKILL.md'))) {
      throw new Error('created user Skill link failed readback verification');
    }
    return fresh.destination;
  } catch (error) {
    if (created && lexists(fresh.destination) && fs.lstatSync(fresh.destination).isSymbolicLink()) {
      fs.unlinkSync(fresh.destination);
    }
    throw error;
  }
}

function buildSkillUnlinkPlan(settings, input) {
  const request = input || {};
  const name = String(request.name || '').trim();
  if (!NAME_PATTERN.test(name)) throw new Error('invalid user Skill name: ' + name);
  const target = path.join(settings.libraryRoot, name);
  if (path.dirname(target) !== path.resolve(settings.libraryRoot)) throw new Error('Skill unlink target escapes the managed user library');
  if (!lexists(target) || !fs.lstatSync(target).isSymbolicLink()) throw new Error('managed user Skill is not a link: ' + name);
  const lock = readSupportedLock(settings);
  if (lock.exists && lock.payload.skills[name]) {
    throw new Error('installer-managed links must use the recycle-bin workflow');
  }
  const targetState = entryState(target);
  return {
    name,
    path: target,
    link_target: fs.readlinkSync(target),
    target_state: targetState,
    actions: [{
      kind: 'skill_link_remove',
      path: target,
      description: '解除用户库链接 ' + target + '；链接源目录保持不变且不会移入回收站',
    }],
  };
}

function skillUnlinkPlanDigest(plan) {
  return sha256(Buffer.from(JSON.stringify({
    name: plan.name,
    path: plan.path,
...
function applySkillUnlink(settings, plan) {
  const fresh = buildSkillUnlinkPlan(settings, { name: plan.name });
  if (skillUnlinkPlanDigest(fresh) !== skillUnlinkPlanDigest(plan)) throw new Error('Skill unlink preview is stale');
  fs.unlinkSync(fresh.path);
  if (lexists(fresh.path)) throw new Error('user Skill link still exists after unlink');
  return fresh.path;
}
...
function buildSkillRemovalPlan(settings, input) {
  ...
  const actions = [{
    ...
    description: mode === 'unlink'
      ? '将用户库链接移入 ASH 回收站 ' + target + '；链接源目录保持不变'
      : '将用户 Skill 移入 ASH 回收站 ' + target,
  }];
  if (lockEntry) {
    actions.push({
...
function buildSkillRemovalPurgePlan(settings, selector) {
  ...
  const payload = transaction.payload;
  if (payload.status !== 'completed') throw new Error('Skill removal transaction is not in the recycle bin: ' + payload.status);
...
function buildSkillRemovalBulkPurgePlan(settings) {
  ...
  if (!removals.length) throw new Error('the Skill recycle bin is empty');
...
function applySkillRemovalBulkPurge(settings, plan) {
  ...
  if (removalBulkPurgePlanDigest(fresh) !== removalBulkPurgePlanDigest(plan)) {
    throw new Error('Skill recycle bin changed after bulk purge preview');
  }
  const deleted = [];
  const failed = [];
...
function buildSkillRemovalRollback(settings, selector) {
  ...
  return {
    ...
    actions: [{
      ...
      path: payload.path,
      description: (payload.mode === 'unlink' ? '恢复用户库软链接 ' : '从 ASH 回收站还原用户 Skill ') + payload.path,
