...
const { buildSkillRemovalRollback } = require('./removal');
...
function transactionRoot(settings, type) {
  if (type === 'repair') return path.join(settings.stateDir, 'transactions');
  if (type === 'removal') return path.join(settings.stateDir, 'removals');
...
function inspectTransaction(settings, type, directory) {
  ...
  if (!payload) obsolete = true;
    ...
    obsolete = payload.version !== 1 || payload.operation !== 'remove-skill';
  }
  return {
    type,
    id,
...
function listTransactions(settings) {
  const records = [];
  ['repair', 'update', 'removal'].forEach(function eachType(type) {
...
function protectedTransactionIds(settings) {
  const protectedIds = { repair: null, update: null, removal: null };
  ...
  try { protectedIds.removal = buildSkillRemovalRollback(settings, 'latest').transaction_id; } catch (error) { /* no safe removal restore */ }
  return protectedIds;
}
...
function buildRetentionPlan(settings, options) {
  ...
  ['repair', 'update', 'removal'].forEach(function classify(type) {
